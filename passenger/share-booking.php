...
// Get driver info - kasama na ang body_number, plate_number, toda_color
    ...
    <div class="two-columns">
        ...
        <div class="info-card">
            ...
            <div class="info-body">
                ...
                <!-- DRIVER SECTION - SOLID like view-booking.php -->
                ...
                <div class="driver-card">
                    <div class="driver-profile">
                        <div class="driver-avatar">
                            <?php 
                            $driver_profile_path = '';
                            $upload_dir_driver = "../uploads/drivers_profile/";
                            
                            if (!empty($driver_info['profile'])) {
                                $physical_path = $_SERVER['DOCUMENT_ROOT'] . '/capstone-part2/uploads/drivers_profile/' . $driver_info['profile'];
                                if (file_exists($physical_path)) {
                                    $driver_profile_web_path = '/capstone-part2/uploads/drivers_profile/' . $driver_info['profile'];
                                } else {
                                    $check_relative = '../uploads/drivers_profile/' . $driver_info['profile'];
                                    if (file_exists($check_relative)) {
                                        $driver_profile_web_path = $check_relative;
                                    } else {
                                        $driver_profile_web_path = '';
                                    }
                                }
                            }
                            ?>
                            <?php if (!empty($driver_profile_web_path) && file_exists($_SERVER['DOCUMENT_ROOT'] . $driver_profile_web_path) || file_exists('../uploads/drivers_profile/' . ($driver_info['profile'] ?? ''))): ?>
                                <img src="<?= htmlspecialchars($driver_profile_web_path ?: ('../uploads/drivers_profile/' . $driver_info['profile'])) ?>" style="width:55px; height:55px; border-radius:50%; object-fit:cover;">
                            <?php else: ?>
                                <div style="width:55px; height:55px; background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:28px;">👨‍✈️</div>
                            <?php endif; ?>
                        </div>
                        <div>
