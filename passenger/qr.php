...
if ($has_driver && !$is_cancelled) {
    // I-join ang users table at kunin ang body_number, plate_number, toda_color
    $driver_stmt = $conn->prepare("
        SELECT u.name, u.email, u.contact, u.profile,
               u.body_number, u.plate_number, u.toda_color,
...
<head>
    ...
    <style>
        ...
        * { font-family: 'NaruMonoDemo', monospace !important; }
        ...
        .vehicle-detail-highlight .vehicle-row {
            margin-bottom: 0.25rem;
        }
...
<div class="ticket-page">
    ...
    <?php 
    // Gamit ang document root para siguradong tama ang path
    $driver_profile_path = '';
    
    if (!empty($driver_info['profile'])) {
        // Kung ang file ay nasa capstone-part2/uploads/drivers_profile/
        $base_path = $_SERVER['DOCUMENT_ROOT'] . '/capstone-part2/uploads/drivers_profile/';
        $driver_profile_path = $base_path . $driver_info['profile'];
        
        // Convert to web-accessible path
        if (file_exists($driver_profile_path)) {
            $web_path = '/capstone-part2/uploads/drivers_profile/' . $driver_info['profile'];
        } else {
            $driver_profile_path = '';
        }
    }
    ?>
    <?php if (!empty($driver_profile_path) && file_exists($driver_profile_path)): ?>
        <img src="<?= htmlspecialchars($web_path) ?>" style="width: 55px; height: 55px; border-radius: 50%; object-fit: cover; border: 2px solid white;">
    <?php else: ?>
        <div style="width: 55px; height: 55px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px;">
            👨‍✈️
        </div>
    <?php endif; ?>
</div>
                            <div class="driver-details">
                                <div class="driver-name">
                                    <?= htmlspecialchars($driver_info['name'] ?? 'Driver') ?>
                                    <?php if (!empty($driver_info['body_number'])): ?>
                                        <span class="body-number-badge">🔢 <?= htmlspecialchars($driver_info['body_number']) ?></span>
                                    <?php endif; ?>
                                </div>
                        ...
                        <!-- VEHICLE DETAILS - with plate number, toda color, body number -->
                        <div class="vehicle-info">
                            <div class="vehicle-row">
                                <span class="label">🚲 Vehicle Type:</span>
                                <span class="value"><?= htmlspecialchars($driver_info['vehicle_type'] ?? 'Tricycle') ?></span>
                            </div>
                            
                            <?php if (!empty($driver_info['plate_number'])): ?>
                            <div class="vehicle-row">
                                <span class="label">🔢 Plate Number:</span>
                                <span class="value"><?= htmlspecialchars($driver_info['plate_number']) ?></span>
                            </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($driver_info['toda_color'])): ?>
                            <div class="vehicle-row">
                                <span class="label">🎨 TODA Color:</span>
                                <span class="value"><?= htmlspecialchars($driver_info['toda_color']) ?></span>
                            </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($driver_info['vehicle_color'])): ?>
                            <div class="vehicle-row">
                                <span class="label">🎨 Vehicle Color:</span>
                                <span class="value"><?= htmlspecialchars($driver_info['vehicle_color']) ?></span>
                            </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($driver_info['vehicle_plate'])): ?>
                            <div class="vehicle-row">
                                <span class="label">📋 Vehicle Plate (Old):</span>
                                <span class="value"><?= htmlspecialchars($driver_info['vehicle_plate']) ?></span>
                            </div>
                            <?php endif; ?>
                            
                            <div class="vehicle-row">
                                <span class="label">📧 Email:</span>
                                <span class="value"><?= htmlspecialchars($driver_info['email'] ?? 'Not available') ?></span>
                            </div>
                        </div>
                        
                        <!-- Vehicle Summary Highlight -->
                        <div class="vehicle-detail-highlight">
                            <div class="vehicle-row">
                                <span class="label">🚖 Tricycle Info:</span>
                                <span class="value">
                                    <?php 
                                    $vehicle_info_parts = [];
                                    if (!empty($driver_info['body_number'])) $vehicle_info_parts[] = "Body: " . $driver_info['body_number'];
                                    if (!empty($driver_info['plate_number'])) $vehicle_info_parts[] = "Plate: " . $driver_info['plate_number'];
                                    if (!empty($driver_info['toda_color'])) $vehicle_info_parts[] = "Color: " . $driver_info['toda_color'];
                                    
                                    if (!empty($vehicle_info_parts)) {
                                        echo htmlspecialchars(implode(" | ", $vehicle_info_parts));
                                    } else {
                                        echo "No additional vehicle info";
                                    }
                                    ?>
                                </span>
                            </div>
...
function getBookingText() {
    ...
    const vehiclePlate = '<?= addslashes($driver_info['plate_number'] ?? $driver_info['vehicle_plate'] ?? 'N/A') ?>';
    const bodyNumber = '<?= addslashes($driver_info['body_number'] ?? 'N/A') ?>';
    const todaColor = '<?= addslashes($driver_info['toda_color'] ?? 'N/A') ?>';
    ...
    if (driverName !== 'Not yet assigned') {
        ...
        text += `🔢 BODY NUMBER: ${bodyNumber}\n`;
        text += `🔢 PLATE NUMBER: ${vehiclePlate}\n`;
        text += `🎨 TODA COLOR: ${todaColor}\n\n`;
    }
