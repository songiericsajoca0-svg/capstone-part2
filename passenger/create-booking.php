...
// LTFRB/MC Taxi TWG Approved Fare Calculation Function
function calculateFare($distance_km, $total_pax = null) {
    // LTFRB/MC Taxi TWG Rate Structure
    $base_fare = 50;              // ₱50 for first 2 kilometers
    $base_distance = 2;           // First 2 kilometers
    $additional_rate = 10;        // ₱10 per additional kilometer
    ...
    // Add charges for distance beyond 2km
    if ($distance_km > $base_distance) {
        $extra_km = ceil($distance_km - $base_distance);  // Round up to next kilometer
        $additional_fare = $extra_km * $additional_rate;
        $total_fare = $base_fare + $additional_fare;
    }
    
    // Required tricycles based on passenger count (max 4 per trike)
    $required_tricycles = 1;
    if ($total_pax) {
    ...
    return [
        ...
        'fare_breakdown' => "₱{$base_fare} (first {$base_distance}km) + ₱{$additional_fare} ({$extra_km}km × ₱{$additional_rate}) = ₱{$total_fare}"
    ];
}
    ...
    if ($total_pax > 4) {
        ...
        if (!$toda_data || !$p_data || !$d_data) {
            ...
            if ($has_highway) {
                ...
                $fare_details = calculateFare($distance, $total_pax);
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="fare-guide">
                ...
                • <strong>Base fare: ₱50</strong> (first 2 kilometers)<br>
                • <strong>Additional: ₱10 per kilometer</strong> (beyond 2km, rounded up)<br>
                • <strong>Same fare</strong> for 1-4 passengers<br>
                • <strong>Maximum: 4 passengers</strong> per tricycle<br>
                • <strong>⚠️ Tricycles are NOT allowed on highways!</strong><br><br>
                <strong>📱 Example:</strong><br>
                • 2km trip = ₱50<br>
                • 3km trip = ₱60 (₱50 + ₱10)<br>
                • 5km trip = ₱80 (₱50 + ₱30)<br>
                • 10km trip = ₱130 (₱50 + ₱80)
            </div>
            ...
            <form method="POST" id="bookingForm">
                ...
                <div id="fare-display-box" class="fare-display-box" style="display:none;">
                    ...
                    <div class="fare-breakdown-details" id="fare-breakdown">
                        ...
                        <p id="base-fare-detail">📌 Base fare: ₱50 (first 2km)</p>
                        <p id="additional-fare-detail">➕ Additional: ₱0 (₱10/km beyond 2km)</p>
                        <p id="distance-detail">📏 Distance: 0 km</p>
                        <p id="calculation-detail">🧮 Computation: </p>
                        <p id="tricycle-detail">🚲 Tricycles needed: 1 (max 4 passengers each)</p>
                        <p id="total-fare-detail" style="color:#059669; font-weight:bold; margin-top:8px;">✅ TOTAL: ₱0</p>
                    </div>
                </div>
...
// LTFRB/MC Taxi TWG Approved Fare Calculation
function calculateNewFare(distanceKm, totalPax) {
    const baseFare = 50;        // ₱50 for first 2km
    const baseDistance = 2;     // First 2 kilometers
    const additionalRate = 10;  // ₱10 per additional km
    ...
    if (distanceKm > baseDistance) {
        extraKm = Math.ceil(distanceKm - baseDistance);
        additionalFare = extraKm * additionalRate;
...
function calculateLogic() {
    ...
    const fareDetails = calculateNewFare(distance, totalPax);
    ...
    // Update breakdown
    document.getElementById('base-fare-detail').innerHTML = `📌 Base fare: ₱${fareDetails.baseFare} (first ${fareDetails.baseDistance}km)`;
    document.getElementById('additional-fare-detail').innerHTML = `➕ Additional: ₱${fareDetails.additionalFare} (₱${fareDetails.additionalRate}/km × ${fareDetails.extraKm}km beyond ${fareDetails.baseDistance}km)`;
    document.getElementById('distance-detail').innerHTML = `📏 Distance: ${distance.toFixed(2)} km`;
    document.getElementById('calculation-detail').innerHTML = `🧮 Computation: ₱${fareDetails.baseFare} + ₱${fareDetails.additionalFare} = ₱${fareDetails.totalFare}`;
    document.getElementById('tricycle-detail').innerHTML = `🚲 Tricycles needed: ${fareDetails.requiredTricycles} (max 4 passengers each)`;
    document.getElementById('total-fare-detail').innerHTML = `✅ TOTAL: ₱${fareDetails.totalFare}`;
