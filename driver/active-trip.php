...
if ($booking_id == 0) {
    ...
    $trip_query = $conn->prepare("
        SELECT b.*, u.name as passenger_name, u.contact as passenger_contact, u.email as passenger_email 
        FROM bookings b
...
$passenger_query = $conn->prepare("SELECT lat, lng, last_location_update, email, name FROM users WHERE id = ? AND role = 'passenger'");
...
<head>
    ...
    <style>
        ...
        * { font-family: 'NaruMonoDemo', monospace !important; }
        ...
        /* Custom fare collection dialog style */
        .fare-collection-dialog {
            text-align: center;
            padding: 20px;
        }
        
        .fare-collection-dialog .fare-icon {
            font-size: 48px;
            margin-bottom: 15px;
        }
        
        .fare-collection-dialog h2 {
            font-size: 22px;
            margin-bottom: 10px;
            color: #1e293b;
        }
        
        .fare-collection-dialog .fare-amount-box {
            background: #10b981;
            color: white;
            padding: 12px;
            border-radius: 16px;
        ...
        .fare-collection-dialog .trip-summary {
            background: #f1f5f9;
            padding: 12px;
            border-radius: 12px;
            margin: 15px 0;
        ...
        .fare-collection-dialog .trip-summary p {
            margin: 5px 0;
        }
...
<body>
    ...
    <div class="trip-panel" id="tripPanel">
        ...
        <div class="panel-header">
            <div class="passenger-section">
                ...
                <div class="passenger-info">
                    ...
                    <span class="passenger-email">📧 <?= htmlspecialchars($trip['passenger_email'] ?? $passenger_location['email'] ?? 'No email provided') ?></span>
                </div>
            </div>
        ...
        <div class="action-buttons">
            ...
            <?php elseif ($current_status == 'PASSENGER PICKED UP'): ?>
                <button class="btn-complete" id="completeBtn" onclick="showFareCollectionDialog(<?= $trip['id'] ?>)">
                    ✅ Complete Trip
                </button>
            <?php endif; ?>
    ...
    <script>
        ...
        // ============================================
        // FARE COLLECTION DIALOG & EMAIL FUNCTIONS
        // ============================================
        
        const tripData = {
            bookingId: <?= $trip['id'] ?>,
            bookingCode: '<?= addslashes($trip['booking_code']) ?>',
            passengerName: '<?= addslashes($trip['passenger_name'] ?? 'Guest') ?>',
            passengerEmail: '<?= addslashes($trip['passenger_email'] ?? $passenger_location['email'] ?? '') ?>',
            pickupLocation: '<?= addslashes($trip['pickup_landmark'] ?? 'Not specified') ?>',
            dropoffLocation: '<?= addslashes($trip['dropoff_landmark'] ?? 'Not specified') ?>',
            totalPax: <?= $trip['total_pax'] ?? $trip['total_passengers'] ?? 1 ?>,
            requiredTricycles: <?= $trip['trike_units'] ?? $trip['required_tricycles'] ?? 1 ?>,
            distanceKm: <?= $trip['distance'] ?? $trip['distance_km'] ?? 0 ?>,
            todaName: '<?= addslashes($trip['toda_name'] ?? 'GoTrike') ?>',
            fareAmount: <?= $trip['fare_amount'] ?? $trip['fare'] ?? 0 ?>,
            paymentMethod: '<?= addslashes($trip['payment_method'] ?? 'cash') ?>'
        };
        
        function showFareCollectionDialog(bookingId) {
            const fareAmount = tripData.fareAmount;
            const formattedFare = new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(fareAmount);
            
            Swal.fire({
                title: '<div style="font-size: 24px;">💰 Collect Fare</div>',
                html: `
                    <div class="fare-collection-dialog">
                        <div class="fare-icon">💵</div>
                        <h2>Have you collected the fare?</h2>
                        <div class="fare-amount-box">
                            ₱ ${fareAmount.toFixed(2)}
                        </div>
                        <div class="trip-summary">
                            <p><strong>📋 Booking Code:</strong> ${tripData.bookingCode}</p>
                            <p><strong>👤 Passenger:</strong> ${escapeHtml(tripData.passengerName)}</p>
                            <p><strong>📧 Email:</strong> ${escapeHtml(tripData.passengerEmail)}</p>
                            <p><strong>📍 From:</strong> ${escapeHtml(tripData.pickupLocation)}</p>
                            <p><strong>🏁 To:</strong> ${escapeHtml(tripData.dropoffLocation)}</p>
                            <p><strong>👥 Passengers:</strong> ${tripData.totalPax}</p>
                            <p><strong>🚲 Tricycles:</strong> ${tripData.requiredTricycles}</p>
                            <p><strong>📏 Distance:</strong> ${tripData.distanceKm.toFixed(2)} km</p>
                            <p><strong>🚖 TODA:</strong> ${escapeHtml(tripData.todaName)}</p>
                        </div>
                        <p style="font-size: 12px; color: #64748b; margin-top: 10px;">
                            ⚠️ Make sure you have received the payment before proceeding.
                        </p>
                    </div>
                `,
                showCancelButton: true,
                confirmButtonText: '✅ Yes, Fare Collected',
                ...
                backdrop: true,
                customClass: {
                    popup: 'fare-collection-swal',
                    confirmButton: 'fare-confirm-btn',
                    cancelButton: 'fare-cancel-btn'
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    // Fare collected - proceed to complete trip and send email
                    completeTripWithEmail(bookingId);
                }
            });
        }
        
        async function completeTripWithEmail(bookingId) {
            Swal.fire({
                title: 'Completing trip...',
                text: 'Please wait while we finalize your trip.',
                allowOutsideClick: false,
                didOpen: () => {
                    Swal.showLoading();
                }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    let successMessage = 'Trip completed successfully!';
                    if (data.email_sent) {
                        successMessage = '✅ Trip Completed! A confirmation email has been sent to the passenger.';
                    } else if (data.email_sent === false) {
                        successMessage = '✅ Trip Completed! However, we could not send the email notification.';
                    }
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        actionDiv.innerHTML = `
                            <button class="btn-complete" onclick="showFareCollectionDialog(${bookingId})">✅ Complete Trip</button>
